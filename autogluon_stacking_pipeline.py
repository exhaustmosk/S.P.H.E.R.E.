...
────────────────────────────────────────────────────────────────────────────
End-to-End Sequential Stacking Pipeline (Python 3.12 + AutoGluon)

Architecture:
  Stage 1 — Strict inner join (2,880 complete cases) + PCA reduction
...
Run with:  .venv312/bin/python autogluon_stacking_pipeline.py
────────────────────────────────────────────────────────────────────────────
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
#  STAGE 1: STRICT INNER JOIN & PCA
# ═══════════════════════════════════════════════════════════════════════════════
print(hdr("Stage 1 · Data Loading & Strict Inner Join"))
...
# ── STRICT INNER JOIN: keep only the first 2880 rows where ALL modalities exist
N_COMPLETE = min(num_df.shape[0], speech_emb.shape[0], facial_emb.shape[0])
print(sub(f"Complete cases (inner join) : {N_COMPLETE}"))

X_num_raw  = num_df[NUMERICAL_FEATURES].values[:N_COMPLETE].astype(np.float32)
X_speech   = speech_emb[:N_COMPLETE].astype(np.float32)
X_facial   = facial_emb[:N_COMPLETE].astype(np.float32)

y_class = num_df["Mental_Health_Status"].values[:N_COMPLETE]
y_class_int = np.array([CLASS_MAP[c] for c in y_class], dtype=np.int64)
y_reg = num_df[REGRESSION_TARGETS].values[:N_COMPLETE].astype(np.float32)

# Class distribution
unique, counts = np.unique(y_class, return_counts=True)
print(sub(f"Class distribution:"))
for cls, cnt in zip(unique, counts):
    print(sub(f"  {cls:20s} {cnt:5d}  ({cnt/N_COMPLETE*100:.1f}%)"))
...
joblib.dump(final_reg, "head_B_regressor_final.joblib")
joblib.dump(scaler, "standard_scaler_final.joblib")
joblib.dump(pca_speech, "pca_speech_final.joblib")
joblib.dump(pca_facial, "pca_facial_final.joblib")
...
plt.tight_layout()
plt.savefig("modality_attribution_bar_chart.png", dpi=300, bbox_inches="tight")
print(ok("Saved modality_attribution_bar_chart.png"))
...
plt.tight_layout()
plt.savefig("top_15_shap_features.png", dpi=300, bbox_inches="tight")
print(ok("Saved top_15_shap_features.png"))

# ── Save final artifacts ─────────────────────────────────────────────────────
joblib.dump(shap_clf, "shap_xgb_proxy.joblib")

print(f"\n{BOLD}{GREEN}{'═'*62}")
print("  End-to-End Sequential Stacking Pipeline Complete.")
print(f"{'═'*62}{RESET}\n")
